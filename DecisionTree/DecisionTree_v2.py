...
# 1. Xác định các đặc trưng quan trọng từ mô hình ban đầu
...
# 2. Huấn luyện mô hình cải tiến
X_train_fs = dt.X_train[selected_features]
X_valid_fs = dt.X_valid[selected_features]
model_v2 = DecisionTreeRegressor(random_state=42, max_depth=3)
model_v2.fit(X_train_fs, dt.y_train)

# 3. Vẽ cấu trúc cây (Lưu vào V2)
plt.figure(figsize=(33,20))
plot_tree(model_v2, feature_names=selected_features, filled=True, rounded=True, fontsize=10)
plt.title("Decision Tree Structure - Improvement V2 (Feature Selection)")
plt.savefig('./Image/V2/Decision_Tree_Structure_v2.png')

# 4. Đánh giá và ghi Log
...
log_results("Model V2 - Feature Selection", mae, mse, rmse, r2)
...
plt.title("Compare Actual vs Predicted Prices - V2")
...
plt.figure(figsize=(10, 6))
plt.title('Important Factors Affecting House Prices - V2')
